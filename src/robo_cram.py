...
from pycram.datastructures.enums import Arms, DetectionTechnique, Grasp, TorsoState, WorldMode
from pycram.datastructures.pose import Pose
from pycram.designators.action_designator import DetectActionDescription, LookAtActionDescription, MoveTorsoActionDescription, NavigateActionDescription, ObjectDesignatorDescription, ParkArmsActionDescription, PickUpActionDescription, PlaceActionDescription, ReachToPickUpActionDescription
from pycram.designators.location_designator import CostmapLocation, SemanticCostmapLocation
from pycram.designators.object_designator import BelieveObject
from pycram.failures import LookAtGoalNotReached, ObjectNotGraspedError, PerceptionObjectNotFound
...
def move_torso(high: bool) -> Response:
    with simulated_robot:
        MoveTorsoActionDescription([TorsoState.HIGH if high else TorsoState.LOW]).resolve().perform()
    return Response(status="success", message=f"Robot torso move to {'high' if high else 'low'} successful")


def spawn_object(obj: Obj, obj_name: str, coordinates: Tuple[float, float, float] = (1.4, 1.0, 0.9),
                 colour: Colour = Colour.DEFAULT) -> Response:
    try:
        position = [float(i) for i in coordinates]
    except ValueError:
        return Response(status="error", message="Coordinates must have exactly 3 real numbers (x,y,z)")
...
def move_robot(coordinates: Tuple[float, float, float] = (0, 0, 0)) -> Response:
    ...
    return Response(status="success", message=f"Robot moved to coordinates {position}", payload={"coordinates": position})
...
def is_object_in_environment(obj_name: str) -> Response:
    ...
    if obj is None:
        return Response(status="error", message=f"Object '{obj_name}' is not in the environment")

    return Response(status="success", message=f"Object '{obj_name}' is in the environment")
...
def is_object_type_in_location(location: Location, obj: Obj) -> Response:
    ...
    if len([_ for _ in object_desig]) == 0:
        return Response(status="error", message=f"Objects of type '{str(obj_type).split('.')[1]}' are not in the environment")

    with simulated_robot:
        try:
            DetectActionDescription(DetectionTechnique.TYPES, object_designator_description=object_desig, region=LOCATIONS[location]).resolve().perform()
            object_found = True
        except PerceptionObjectNotFound:
            object_found = False

    return Response(status="success" if object_found else "error", message=f"Objects of type '{str(obj_type).split('.')[1]}' are{'' if object_found else ' not'} in the location")
...
def is_object_in_location(location: Location, obj_name: str) -> Response:
    ...
    return Response(status="success" if object_found else "error", message=f"Object '{obj_name}' is{'' if object_found else ' not'} in the location")
...
def is_object_visible_to_robot(obj_name: str) -> Response:
    ...
    if visible is None:
        return Response(status="error", message=f"Object '{obj_name}' is in the environment but not visible to the robot")

    return Response(status="success", message=f"Object '{obj_name}' is visible to the robot")
...
def is_object_reachable_by_robot(obj_name: str) -> Response:
    ...
    if not reachable.reachable_arms:
        return Response(status="error", message=f"Object '{obj_name}' is in the environment but not reachable by the robot")

    return Response(status="success", message=f"Object '{obj_name}' is reachable by the robot")
...
def look_at_object(obj_name: str) -> Response:
    ...
    if not looking_at_object:
        return Response(status="error", message=f"Robot vision failed to reach '{obj_name}'")
...
def pick_and_place(obj_name: str, destination: Location) -> Response:
    ...
    if pickup_arm == None:
        ...
        return Response(status="error", message=f"No reachable arms to pick up object '{obj_name}'")

    with simulated_robot:
        NavigateActionDescription(target_location=Pose(pickup_pose.pose.position, pickup_pose.pose.orientation)).resolve().perform()

        grasped = False
        for grasp in [Grasp.FRONT, Grasp.BACK, Grasp.RIGHT, Grasp.LEFT, Grasp.TOP, Grasp.BOTTOM]:
            try:
                PickUpActionDescription(object_designator=obj, arm=pickup_arm, grasp_description=grasp).resolve().perform()
                grasped = True
    ...
    if not grasped:
        ...
        return Response(status="error", message=f"The object '{obj_name}' could not be grasped")

    with simulated_robot:
        obj_dest_location = SemanticCostmapLocation(link_name=LOCATIONS[destination], part_of=env, for_object=obj).resolve()
        place_location = CostmapLocation(target=obj_dest_location.pose, reachable_for=robot, reachable_arm=pickup_arm).resolve()

        NavigateActionDescription(target_location=place_location.pose).resolve().perform()
        PlaceActionDescription(obj, target_location=obj_dest_location.pose, arm=pickup_arm).resolve().perform()
        ParkArmsActionDescription([Arms.BOTH]).resolve().perform()
        MoveTorsoActionDescription([TorsoState.LOW]).resolve().perform()

    return Response(status="success", message=f"Object '{obj_name}' successfully moved to '{LOCATIONS[destination]}'")
