...
from interactive_sim import (
    initialize_world, create_objects, add_objects, run_robot_actions,
    capture_camera_image, demo_camera, demo_advanced_camera_functions, 
    demo_object_distances, exit_world, spawn_objects
)
...
try:
    ...
    print("Warning: robot_actions.api module not found. Robot movement functionality will be disabled.")
...
app = FastAPI(title="PyCRAM Camera API", description="API for interacting with PyCRAM camera and robot functions")
...
async def get_js():
    try:
        ...
        raise HTTPException(status_code=404, detail=f"JavaScript file not found: {str(e)}")
...
async def api_initialize_world():
    try:
        ...
        if interactive_sim.world is None:
            return {"status": "error", "message": "Failed to initialize world. The world variable is still None."}
        ...
        for obj in world.objects:
            if hasattr(obj, 'name'):
                ...
                obj_info = {
                    ...
                    "position": [position.x, position.y, position.z] if position else "unknown"
                }
            ...
            if request.save_path:
                try:
                    ...
                    if os.path.exists(saved_path):
                        ...
                        save_result = f"Warning: Unable to verify saved file at {saved_path}"
                except Exception as save_err:
                    save_result = f"Warning: Error verifying saved file: {str(save_err)}"
        except Exception as e:
            return {"status": "error", "message": f"Failed to capture camera image: {str(e)}"}
...
async def api_visualize_depth_map(colormap: str = "plasma"):
    try:
        ...
        try:
            ...
            return {"status": "error", "message": f"Failed to initialize camera utilities: {str(e)}"}
        ...
        try:
            ...
            plt.close()
            return {"status": "error", "message": f"Failed to visualize depth map: {str(e)}"}
...
async def api_identify_objects():
    try:
        ...
        # Identify objects
        try:
            visible_objects = pr2_camera_utils.identify_objects_in_view(display_segmentation=False)
        except Exception as e:
            return {"status": "error", "message": f"Failed to identify objects: {str(e)}"}
        ...
        if object_list:
            ...
            for obj in object_list:
                message += f"- {obj['name']} (ID: {obj['id']}, Pixels: {obj['pixel_count']})\n"
        else:
        ...
        try:
            ...
            for obj_data in object_list:
                ...
                plt.annotate(f"{name} ({px_count} px)", 
            ...
            plt.close()
            return {"status": "error", "message": f"Failed to visualize object identification: {str(e)}"}
...
async def api_check_object_visibility(request: ObjectVisibilityRequest):
    try:
        ...
        try:
            ...
            return {"status": "error", "message": f"Failed to check object visibility: {str(e)}"}
        ...
        try:
            ...
            plt.close()
            return {"status": "error", "message": f"Failed to visualize object visibility: {str(e)}"}
...
async def api_find_occluding_objects(object_name: str):
    try:
        ...
        try:
            occluding_objects = pr2_camera_utils.find_occluding_objects(object_name, display=False)
        except Exception as e:
            return {"status": "error", "message": f"Failed to find occluding objects: {str(e)}"}
        
        # Create visualization
        fig = plt.figure(figsize=(10, 8))
        try:
            occlusion_image = pr2_camera_utils.visualize_occlusions(object_name, display=False)
        except Exception as e:
            plt.close()
            return {"status": "error", "message": f"Failed to visualize occlusions: {str(e)}"}
...
async def api_estimate_distances():
    try:
        ...
        try:
            ...
            return {"status": "error", "message": f"Failed to estimate distances: {str(e)}"}
        ...
        try:
            ...
            plt.close()
            return {"status": "error", "message": f"Failed to visualize distances: {str(e)}"}
...
async def api_look_at_object(request: LookAtObjectRequest):
    try:
        ...
        try:
            ...
            plt.close()
            return {"status": "error", "message": f"Failed to capture view after looking at object: {str(e)}"}
...
async def api_scan_environment(request: ScanEnvironmentRequest):
    try:
        ...
        # Scan environment
        try:
            scan_images = pr2_camera_utils.scan_environment(angles=request.angles, display=False)
        except Exception as e:
            return {"status": "error", "message": f"Failed to scan environment: {str(e)}"}
        ...
        for i, img in enumerate(scan_images):
            try:
                ...
                plt.close()
                return {"status": "error", "message": f"Failed to process scan image {i+1}: {str(e)}"}
...
async def api_object_distances(request: DistanceRequest):
    try:
        ...
        try:
            ...
            if not distances:
                return {"status": "success", "message": "No distances found between objects", "distances": []}
            ...
            return {"status": "error", "message": f"Failed to calculate object distances: {str(e)}"}
        ...
        try:
            ...
            plt.close()
            return {"status": "error", "message": f"Failed to display object distances: {str(e)}"}
...
async def api_visualize_3d_distances(request: Visualization3DRequest):
    try:
        ...
        try:
            ...
            plt.close()
            return {"status": "error", "message": f"Failed to visualize 3D distances: {str(e)}"}
...
async def api_move_robot(request: RobotMoveRequest):
    try:
        ...
        if not has_robot_movement:
            return {"status": "error", "message": "Robot movement functionality is not available."}
        ...
        try:
            ...
            return {"status": "success", "message": f"Robot moved to location {target_location}", "result": result}
        except Exception as e:
